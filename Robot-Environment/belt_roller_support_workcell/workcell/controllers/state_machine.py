...
from .prim_utils import magnet_attach, magnet_detach
...
class WorkcellStateMachine:
    ...
    def _update_ur5_assembly(self, dt: float):
        self._ur5_t += dt
        ur5 = self.world.ur5

        if self._ur5_step == 0:
            ur5.set_goal([0.55, -0.25, 0.45], [0, 0, 0, 1])
            if self._ur5_t > 1.5:
                self._ur5_step += 1
                self._ur5_t = 0.0

        elif self._ur5_step == 1:
            ur5.set_goal([0.55, -0.25, 0.32], [0, 0, 0, 1])
            if self._ur5_t > 1.5:
                self._ur5_step += 1
                self._ur5_t = 0.0

        elif self._ur5_step == 2:
            ur5.set_goal([0.55, -0.25, 0.45], [0, 0, 0, 1])

def _update_ur10_assembly(self, dt: float):
    self.t += dt
    ur10 = self.world.ur10

    # Tool pointing DOWN (Z-)
    q_down = [0.7071, 0.0, 0.7071, 0.0]

    # ----------------------------
    # STEP 0: Move above pick
    # ----------------------------
    if self.step == 0:
        ur10.set_goal([0.70, 0.20, 0.45], q_down)
        if self.t > 1.2:
            self.step = 1
            self.t = 0.0

    # ----------------------------
    # STEP 1: Move down & PICK
    # ----------------------------
    elif self.step == 1:
        ur10.set_goal([0.70, 0.20, 0.32], q_down)
        if self.t > 1.0:
            magnet_attach(
                part_prim_path=f"/World/{self.picked_part}",
                ee_prim_path="/World/ur10_short_suction/ee_link"
            )
            self.step = 2
            self.t = 0.0
    ...
    # ----------------------------
    # STEP 3: Move above place
    # ----------------------------
    elif self.step == 3:
        ur10.set_goal([1.10, -0.30, 0.45], q_down)
        if self.t > 1.2:
            self.step = 4
            self.t = 0.0

    # ----------------------------
    # STEP 4: Place & RELEASE
    # ----------------------------
    elif self.step == 4:
        ur10.set_goal([1.10, -0.30, 0.32], q_down)
        if self.t > 1.0:
            magnet_detach(
                part_name=self.picked_part,
                target_parent_path="/World/ConveyorBelt_A23"
            )
            self.step = 5
            self.t = 0.0
