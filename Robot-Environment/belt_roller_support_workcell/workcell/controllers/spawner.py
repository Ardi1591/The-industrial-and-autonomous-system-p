...
from ..prim_utils import Pose, add_reference, ensure_xform, set_world_pose, delete_prim
...
class Spawner:
    ...
    def spawn(
        ...
        # IMPORTANT: add_reference(dst_prim_path, usd_path)
        usd_ref = str(asset.usd_path).replace('\\', '/')
        add_reference(prim_path, usd_ref)

        # prim_utils.set_world_pose expects a Pose object (position + orientation)
        pose = Pose(tuple(float(x) for x in position), tuple(float(x) for x in orientation_xyzw))
        set_world_pose(prim_path, pose)

        # Optional: disable USD instancing under this spawned prim so you can edit
        # (and sometimes get more predictable collision behavior).
        self._disable_instancing_recursive(prim_path)

        log.debug("Spawned %s from %s at %s", prim_path, asset.usd_path, position)
        return SpawnedObject(prim_path=prim_path, asset=asset)

    @staticmethod
    def _disable_instancing_recursive(root_prim_path: str) -> None:
        """Set instanceable=False for root prim and all descendants.

        NOTE: This does *not* delete the prim named "MeshInstance"; it just disables instancing.
        If the referenced USDZ was authored with a MeshInstance prim name, that name will remain.
        """
        try:
            ...
            # Safe to ignore if pxr/omni aren't available (e.g., linting outside Isaac).
            return
