...
from .prim_utils import Pose, find_anchorpoints, get_world_pose, set_world_pose, _require_isaac
...
class WorkcellWorld:
    ...
    def _ensure_physics_scene(self) -> None:
        """Ensure a USD PhysicsScene exists so World.step() has a physics context.

        Some USD assets do not include a PhysicsScene prim; in that case Isaac's
        SimulationContext ends up with no physics context and World.step() fails
        with: AttributeError: 'NoneType' object has no attribute 'get_physics_dt'.
        """
        _require_isaac()
        try:
        ...
        for prim in stage.Traverse():
            if prim.GetTypeName() in ("PhysicsScene", "PhysxScene"):
                return
        ...
        # Define physics scene + basic gravity
        ...
        # Apply PhysX API so PhysX picks it up
        try:
            PhysxSchema.PhysxSceneAPI.Apply(stage.GetPrimAtPath(scene_path))
        except Exception:
            pass

    def _ensure_simulation_ready(self) -> None:
        """Ensure the Isaac timeline is playing and physics is initialized."""
        _require_isaac()
        # 1) Ensure a physics scene exists
        try:
            self._ensure_physics_scene()
        except Exception as e:
            log.warning("Could not ensure physics scene: %s", e)

        # 2) If running from Script Editor, timeline may be stopped. Start it.
        try:
            ...
            # Pump a few frames to let SimulationContext/PhysX initialize
            app = omni.kit.app.get_app()
            for _ in range(3):
                app.update()
        except Exception as e:
            log.warning("Could not start timeline/update app: %s", e)
    def load(self) -> None:
        ...
        self._world.reset()
        # In GUI Script Editor the timeline is often stopped; start it and initialize physics
        self._ensure_simulation_ready()
    ...
    def run_until_done(self) -> None:
        ...
        # Ensure timeline/physics are initialized (fixes NoneType get_physics_dt on some stages)
        self._ensure_simulation_ready()
        sim_seconds = 0.0
